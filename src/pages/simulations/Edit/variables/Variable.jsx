...
import { emptyVariableName, emptyVariableTitle, accordionStyle, fixNumber, strToNumber } from '../util'
...
export function Variable({ simulation, variable, expanded, flipExpand, duplicate }) {
	...
	return <Accordion sx={accordionStyle} expanded={expanded} onChange={() => flipExpand()}>
		...
		<AccordionDetails key="details" sx={{ py: 0, my: -2 }}>
			...
			<div style={{ display: 'flex', flexFlow: 'row nowrap', gap: '1rem', marginBottom: '-1.4rem', marginTop: '-1.4rem' }}>
				<div style={{ width: '50%' }}>
